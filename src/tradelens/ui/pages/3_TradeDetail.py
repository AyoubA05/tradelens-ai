...
from src.tradelens.ui.components.corrections_sidebar import (
...
if not api_key_ok:
    st.warning(
        "ANTHROPIC_API_KEY is not set. Add it to your secrets/.env to enable AI analysis."
    )
    ...
    if analyze_btn:
        ...
        with st.spinner("Analyzing chart… this may take 10–20 seconds."):
            try:
                ...
                st.error(f"The AI returned an unreadable response: {exc}")
...
if analysis is not None:
    ...
    with col_a:
        bias_idx = (
            BIAS_OPTIONS.index(analysis.bias) if analysis.bias in BIAS_OPTIONS else 0
        )
    ...
    st.markdown(
        "**SMC/ICT Setup** — editable; AI proposals shown only where you haven't set a value"
    )
    ...
    if saved_grading is None:
        ...
        st.markdown(
            f"**AI Grade:** {grade_icon} **{ai_grade}** &nbsp;|&nbsp; Score: **{saved_grading.get('score', '—')}/10**"
        )
        ...
        with btn_col:
            ...
            if st.button("Save Override", key="save_override_btn"):
                ...
                record_correction(
                    selected_id, analysis.id, "grade", trade.ai_grade, resolved_override
                )
