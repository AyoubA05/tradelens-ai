"""One low-data policy shared by the Streamlit app and the API.
...
def sample_state(df: Optional[pd.DataFrame]) -> SampleState:
...
def enough_categories(
    df: Optional[pd.DataFrame], column: str, minimum: int = _MIN_CATEGORIES
) -> bool:
