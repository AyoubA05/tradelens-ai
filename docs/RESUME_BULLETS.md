...
- Built an AI post-trade trading journal using Streamlit, SQLAlchemy, and Anthropic's claude-fable-5 (1M-token context, adaptive thinking effort) — routing six distinct AI pipelines through a single audited client with prompt caching and a DEMO_MODE that runs the entire app at zero API cost.
