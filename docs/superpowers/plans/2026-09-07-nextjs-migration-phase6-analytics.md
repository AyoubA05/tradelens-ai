...
**Two vocabularies for an undefined figure now exist, and that is a deliberate deferral.** `format.undefinedReason` (Overview) returns short labels for a KPI tile and **collapses `undefined_nan`, `undefined_no_sample` and any unknown state into one string, "Not enough data"**. Analytics needs the five states distinguishable — "there is nothing to measure" and "this could not be computed" call for different responses from a trader — so `metric-value.tsx` carries its own sentence map. Two surfaces with different space budgets is a defensible reason to word things differently; **collapsing two genuinely different states into one label on Overview is not**, and that is a real (pre-existing) shortcoming rather than a styling choice. Fixing it means touching Overview, which is outside this phase. Recorded here so it is a decision rather than an accident.

`format.ts` also has no percent/ratio/number formatter — only `money`. C1 inlines those using the exact expressions `overview/kpi-row.tsx` already uses. Consolidating them into `format.ts` is a refactor of a shared module and belongs in its own task.
