...
**Two consequences of the text-free ticket, recorded rather than designed away** (Group B review):
- **A retry needs a fresh `client_turn_id`, and it counts against the limit.** `enqueue_with_limit` returns the existing row whatever its status, so a ticket left `failed` answers `DuplicateTurn` for that id forever. That is the safe posture — a failed paid call is never silently re-billed under the same key — and the browser mints a new id for a deliberate retry. A ticket stranded `running` (process killed between the insert and the `finally`) is permanent in the same way and occupies one of the 60 slots in the 24h window; there is no reaper. If that ever bites in practice, the fix is a sweeper that fails `running` partner tickets older than a few minutes, not a looser duplicate rule.
- **A retry after a lost response may create a second paid provider call — deliberately.** If the provider answered (and was billed) but the response never reached the browser — a dropped connection, a relay or platform timeout, a closed tab — the trader's retry is a new attempt with a fresh `client_turn_id`, and it calls the provider again. The Partner intentionally does not persist reply content anywhere, so the server holds no answer it could return in place of a second call; storing one would reintroduce the server-side transcript this phase forbids. **Duplicate protection guarantees one paid call per in-flight send** (one `client_turn_id`: a double-submit, two tabs racing the same send, a replayed request) — **not one paid call per question.** Both calls are usage-logged (D6) and both count against the owner's limit. The web client never retries on its own: a second call only ever follows the trader pressing "Ask that again" or asking again.
- **The loser of two concurrent identical submits gets no answer.** The ticket carries no text, so a `DuplicateTurn` cannot return the winner's reply. Group D's copy must say so plainly ("this question is already being answered in another tab"), never invent a reply, and never re-send.
