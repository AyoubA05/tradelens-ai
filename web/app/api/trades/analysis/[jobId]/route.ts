...
import { fetchAnalysisJob } from "@/lib/app/trade-analysis";
import {
  analysisRelayError,
  ANALYSIS_NO_STORE,
...
/**
 * `GET /api/trades/analysis/{jobId}` — poll one analysis job.
 *
...
export async function GET(
  ...
  try {
    const job = await fetchAnalysisJob(auth.token, jobId);
