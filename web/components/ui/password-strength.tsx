...
/**
 * 21.dev PasswordStrength — the component as supplied, with one import change
 * and one palette change.
 *
 *  - `motion/react` -> `framer-motion`. Same library (motion is its successor)
 *    and the same exports; installing both would put two animation runtimes in
 *    one bundle. No animation, timing, or behaviour is altered.
 *  - the stone/emerald/amber/red tones are mapped onto TradeLens colours.
 *
 * Rules, spring constants, the guessable-pattern detection, the debounce, the
 * ARIA meter and the live region are all unchanged from the source.
 *
 * **This meter is UX only.** The identical policy is enforced independently
 * server-side; a request that bypasses the browser is rejected there.
 */
...
// Retoned for TradeLens: the accent carries "safe", and the caution/danger
// steps keep their semantic distance so the meter still reads at a glance.
const TONES = {
  none: { bar: "bg-white/15", text: "text-muted" },
  danger: { bar: "bg-red-500", text: "text-red-400" },
  caution: { bar: "bg-amber-500", text: "text-amber-400" },
  safe: { bar: "bg-accent", text: "text-accent" },
} as const;
    ...
    <div className={`w-full ${className}`}>
      ...
      >
        {Array.from({ length: max }, (_, i) => (
          <div
            key={i}
            className="relative h-1.5 overflow-hidden rounded-[2px] bg-white/10"
          >
            <motion.span
        ...
        <ul className="mt-3 grid gap-1.5">
          {evaluated.map((rule) => (
            <li key={rule.id} className="flex items-center gap-2">
              <span className="relative grid size-[14px] shrink-0 place-items-center rounded-[4px] border border-white/[0.16] text-bg">
                <motion.span
