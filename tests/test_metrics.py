...
def test_emotion_vs_rr_nan_excluded():
    ...
    assert calm["avg_rr_realized"] == pytest.approx(
        3.0, abs=1e-6
    )  # NaN excluded from mean
...
def test_killzone_performance_excludes_null_killzone():
    df = pd.DataFrame(
        {"killzone": [None, "ny_am"], "result": ["Win", "Win"], "pnl": [10.0, 20.0]}
    )
...
def test_calendar_daily_pnl_single_trade():
    df = pd.DataFrame(
        {"trade_date": ["2025-01-07"], "result": ["Loss"], "pnl": [-25.0]}
    )
    res = calendar_daily_pnl(df, 2025, 1)
    assert len(res) == 1
...
# ---------------------------------------------------------------------------
# consistency_score (Week 5 Phase 7)
# ---------------------------------------------------------------------------


def _dates(n):
    return [f"2026-06-{i:02d}" for i in range(1, n + 1)]


def test_consistency_score_fewer_than_5_trades_returns_0():
    from src.tradelens.services.metrics import consistency_score

    df = pd.DataFrame(
        {
            "trade_date": _dates(4),
            "followed_rules": [1, 1, 1, 1],
            "mistake_tags": ["[]"] * 4,
            "user_grade": ["A"] * 4,
        }
    )
    assert consistency_score(df) == 0.0


def test_consistency_score_perfect_is_100():
    """Rules all followed, no mistakes, grades improving F→A → 100."""
    from src.tradelens.services.metrics import consistency_score

    df = pd.DataFrame(
        {
            "trade_date": _dates(5),
            "followed_rules": [1, 1, 1, 1, 1],
            "mistake_tags": ["[]"] * 5,
            "user_grade": ["F", "D", "C", "B", "A"],
        }
    )
    assert consistency_score(df) == 100.0


def test_consistency_score_all_broken_is_zero():
    """No rules followed, mistakes everywhere, grades declining A→F → 0."""
    from src.tradelens.services.metrics import consistency_score

    df = pd.DataFrame(
        {
            "trade_date": _dates(5),
            "followed_rules": [0, 0, 0, 0, 0],
            "mistake_tags": ['["FOMO"]'] * 5,
            "user_grade": ["A", "B", "C", "D", "F"],
        }
    )
    assert consistency_score(df) == 0.0


def test_consistency_score_weights_blend_with_neutral_trend():
    """Rules followed + clean + flat grades → 40 + 35 + (25*0.5) = 87.5."""
    from src.tradelens.services.metrics import consistency_score
